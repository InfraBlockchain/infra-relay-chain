...
//! The infrabs runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
impl_runtime_weights!(infrabs_runtime_constants);

// Make the WASM binary available.
#[cfg(feature = "std")]
include!(concat!(env!("OUT_DIR"), "/wasm_binary.rs"));

// infrabs version identifier;
/// Runtime version (infrabs).
#[sp_version::runtime_version]
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("infrabs"),
	impl_name: create_runtime_str!("parity-infrabs"),
	authoring_version: 0,
	spec_version: 9370,
	impl_version: 0,
...
parameter_types! {
	pub Prefix: &'static [u8] = b"Pay DOTs to the infrabs account:";
}
...
parameter_types! {
	...
	// infrabs Genesis was on May 26, 2020.
...
impl frame_support::traits::OnRuntimeUpgrade for InitiateNominationPools {
	fn on_runtime_upgrade() -> frame_support::weights::Weight {
		...
		if pallet_nomination_pools::MaxPools::<Runtime>::get().is_none() {
			...
			log::info!(target: "runtime::infrabs", "pools config initiated 🎉");
			<Runtime as frame_system::Config>::DbWeight::get().reads_writes(1, 5)
		} else {
			log::info!(target: "runtime::infrabs", "pools config already initiated 😏");
...
sp_api::impl_runtime_apis! {
	...
	impl frame_try_runtime::TryRuntime<Block> for Runtime {
		fn on_runtime_upgrade(checks: frame_try_runtime::UpgradeCheckSelect) -> (Weight, Weight) {
			log::info!("try-runtime::on_runtime_upgrade infrabs.");
...
mod remote_tests {
	...
	async fn run_migrations() {
		...
		let transport: Transport =
			var("WS").unwrap_or("wss://rpc.infrabs.io:443".to_string()).into();
